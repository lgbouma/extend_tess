...
from plot_single_camera import line, plane
...
class fourCamView(object):
    ...
    def __init__(self, cam1):
        ...
        if atPole == False:
            theta = np.array([90*radPerDeg-1.5*fourCamView.camAngleSep, 90*radPerDeg-0.5*fourCamView.camAngleSep, \
                              90*radPerDeg+0.5*fourCamView.camAngleSep, 90*radPerDeg+1.5*fourCamView.camAngleSep])
        ...
        self.cam5pos = np.zeros([3, patch.numQuad*self.cam1.nx, self.cam1.nx])
        ...
        for j in range(np.shape(self.cam1pos)[1]):
            for k in range(np.shape(self.cam1pos)[2]):
                self.cam5pos[:,j,k] = np.dot(rotation_matrix(axis,theta[0]), self.cam1pos[:,j,k])
        ...
        axis = [0,1,0] # will rotate about y-axis

        if atPole == False:
            theta = np.array([23*radPerDeg, 23*radPerDeg,
                              23*radPerDeg, 23*radPerDeg,])

        for j in range(np.shape(self.cam1pos)[1]):
            for k in range(np.shape(self.cam1pos)[2]):
                self.cam5pos[:,j,k] = np.dot(rotation_matrix(axis,theta[0]),
                                             self.cam5pos[:,j,k])
                self.cam2pos[:,j,k] = np.dot(rotation_matrix(axis,theta[1]),
                                             self.cam2pos[:,j,k])
                self.cam3pos[:,j,k] = np.dot(rotation_matrix(axis,theta[2]),
                                             self.cam3pos[:,j,k])
                self.cam4pos[:,j,k] = np.dot(rotation_matrix(axis,theta[3]),
                                             self.cam4pos[:,j,k])
...
def phiRotation(view1, numPhiRot):
    ...
    """
    phiRot = 360./26.*radPerDeg*numPhiRot # 27.69 deg rotations (13/hemi; watch for int division!)
    zAxis = [0,0,1]
...
if __name__ == '__main__':
    ...
    #x, y = np.linspace(xMin, xMax, 101), np.linspace(yMin, yMax, 101) #FIXME
    ...
    v.append(view_ini)
    #for i in range(1,26): #NOTE: too dense to look at
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('ralines.png',dpi=256)
    #plt.savefig('eshort.pdf')
