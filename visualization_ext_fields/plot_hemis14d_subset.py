...
def longLatLines(polarProj, ax):
    ...
    for i in range(len(theta_c)):
        ...
        ax.plot(latLine.x, latLine.y, latLine.z, c=colorStr, alpha=0.1, linestyle=ls)
    for i in range(len(phi_c)):
        longLine = longLines(phi_c[i])
        ax.plot(longLine.x, longLine.y, longLine.z, c='black', alpha=0.1)    
...
class fourCamView(object):
    ...
    def __init__(self, cam1):
        ...
        self.allCams = np.concatenate((self.cam1pos, self.cam2pos, self.cam3pos, self.cam4pos),axis=1)
...
if __name__ == '__main__':
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('hemis14d_lowelev_subset.png',dpi=256)
    #plt.savefig('hemis14d.pdf')
