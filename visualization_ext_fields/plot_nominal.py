...
class fourCamView(object):
    ...
    def __init__(self, cam1):
        ...
        if not atPole:
            theta = np.array([0+radPerDeg*90-1.5*fourCamView.camAngleSep, 90.*radPerDeg-0.5*fourCamView.camAngleSep, \
                          90.*radPerDeg+0.5*fourCamView.camAngleSep, 90.*radPerDeg+1.5*fourCamView.camAngleSep]) 
        if atPole:
            theta = np.array([-1.5*fourCamView.camAngleSep, -0.5*fourCamView.camAngleSep, 0.5*fourCamView.camAngleSep, \
                          1.5*fourCamView.camAngleSep])
...
if __name__ == '__main__':
    ...
    x, y = np.linspace(xMin, xMax, 11), np.linspace(yMin, yMax, 11) #FIXME
    ...
    view_ini = fieldRotation(view_ini, [0,1,0], 90.*radPerDeg)

    v = []
    v.append(view_ini) # for testing
    for i in range(1,7):
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('elong.png',dpi=256)
    plt.savefig('elong.pdf')
